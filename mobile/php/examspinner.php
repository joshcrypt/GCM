...
if(isset($_REQUEST['weekday'])&&isset($_REQUEST['course'])&&isset($_REQUEST['year'])){
	...
	$sql="SELECT * FROM exam_timetable WHERE Course_Name='";
	...
	if($num_rows >0){
		...
		$c = array(array('error' => 'REQUESTED TIMETABLE DOES NOT EXIST', 
						 'Unit_Name'=>'not available','Start_Time'=>'not available','Stop_Time'=>'not available','Venue' =>'not available','Exam_Day'=>'not available'));
		echo json_encode($c);
